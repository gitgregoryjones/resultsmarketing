<?php
/**
 * Template Name: RM Services (Published)
 * Description: WordPress template that renders the published services.html output.
 */
...
rm_render_published_html('services.html');

// Placeholder for repeating services data (existing service cards remain in published HTML).
$rm_services = [
    [
        'title' => 'Service Name 1',
        'description' => 'Existing service content can map to a repeater field.',
        'link' => '#',
    ],
    [
        'title' => 'Service Name 2',
        'description' => 'Use dynamic service data while preserving current visual layout.',
        'link' => '#',
    ],
];

/*
<?php if (!empty($rm_services)) : ?>
<section class="service-list">
    <?php foreach ($rm_services as $service) : ?>
        <article class="service-item">
            <h2><?php echo esc_html($service['title']); ?></h2>
            <p><?php echo esc_html($service['description']); ?></p>
            <a href="<?php echo esc_url($service['link']); ?>">Learn more</a>
        </article>
    <?php endforeach; ?>
</section>
<?php endif; ?>
