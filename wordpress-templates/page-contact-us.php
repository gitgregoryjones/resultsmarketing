<?php
/**
 * Template Name: RM Contact Us (Published)
 * Description: WordPress template that renders the published contact-us.html output.
 */
...
rm_render_published_html('contact-us.html');

// Placeholder for repeating office/contact data (existing contact data remains in published HTML).
$rm_contact_locations = [
    [
...
/*
<?php if (!empty($rm_contact_locations)) : ?>
<section class="contact-location-list">
    <?php foreach ($rm_contact_locations as $location) : ?>
        <article class="contact-location-item">
            <h2><?php echo esc_html($location['office_name']); ?></h2>
            <p><?php echo esc_html($location['phone']); ?></p>
            <p><?php echo esc_html($location['email']); ?></p>
