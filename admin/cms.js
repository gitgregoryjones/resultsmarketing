(function () {
  ...
  function getYoutubeEmbedUrl(urlValue = '') {
    ...
    const normalizedId = videoId.replace(/[^a-zA-Z0-9_-]/g, '').trim();
  ...
  function updateImagePreview(src, type = selectedType) {
    ...
    if (type === 'video') {
      ...
      if (youtubeEmbed) {
        ...
        previewFrame.className = 'cms-image-preview__video';
        previewFrame.src = youtubeEmbed;
        previewFrame.setAttribute('frameborder', '0');
        previewFrame.setAttribute('allowfullscreen', 'true');
        previewFrame.setAttribute('title', 'YouTube video preview');
  ...
  function getImageValue(el, key, type = 'image') {
    ...
    if (type === 'video') {
      ...
      if (nestedVideo) return nestedVideo.getAttribute('src') || '';
      const nestedFrame = el.querySelector ? el.querySelector('iframe') : null;
      return nestedFrame?.getAttribute('src') || '';
    }
      ...
      if (youtubeEmbed) {
        ...
        if (!iframeNode && (el.tagName === 'IMG' || el.tagName === 'VIDEO')) {
          ...
          Array.from(el.attributes || []).forEach((attribute) => {
            ...
            iframeNode.setAttribute(name, attribute.value);
          });
          if (selectedElement === el) selectedElement = iframeNode;
          el.replaceWith(iframeNode);
        }
        if (!iframeNode) {
          iframeNode = document.createElement('iframe');
          if (el.tagName === 'IFRAME') {
            el.replaceWith(iframeNode);
            selectedElement = iframeNode;
          } else {
            el.innerHTML = '';
            el.appendChild(iframeNode);
          }
        }
        iframeNode.src = youtubeEmbed;
        iframeNode.setAttribute('frameborder', '0');
        iframeNode.setAttribute('allowfullscreen', 'true');
        iframeNode.setAttribute('title', 'Embedded video');
        return;
      }
      let hostElement = el;
