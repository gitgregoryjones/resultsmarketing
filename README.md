...
## How it works
1. HTML contains elements with `data-cms-text="key"` or `data-cms-image="key"` attributes.
2. The Node server reads `content.json` and renders the page with matching text replacements and image sources so content is visible to crawlers and users on first paint.
...
## Files
- `index.html` – Demo page using tagged text and image elements.
- `content.json` – Content values keyed by CMS attributes, persisted on save (plus `__tags` for selectors/type metadata).
- `cms.js` – Inline CMS logic (hydration, edit mode, sidebar UI, image uploads/URLs, server sync).
...
- `POST /api/content` – Accepts `{ key, value, path, type, image, originalOuterHTML, updatedOuterHTML }`, updates `content.json` (including tag selectors and type under `__tags`), saves uploaded images to `/images`, and rewrites `index.html` with new tags when provided.
...
- Images can be swapped by uploading a file (persisted to `/images`) or pasting a remote URL.
